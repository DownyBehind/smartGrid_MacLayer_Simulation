...
void SlacApplication::initialize()
{
    ...
    dcTimeoutSignal = registerSignal("dcTimeout");
    dcGapViolationSignal = registerSignal("dcGapViolation");
    dcRttSignal = registerSignal("dcRtt");
    
    // Force immediate output
    printf("=== SLAC APPLICATION INITIALIZE ===\n");
    printf("[%.3f] Node %d (%s): Initializing - calling startSlac\n", simTime().dbl(), nodeId, nodeType.c_str());
    printf("dcLoopEnabled = %s\n", dcLoopEnabled ? "true" : "false");
    printf("recordSlacMessages = %s\n", recordSlacMessages ? "true" : "false");
    printf("recordDcCycles = %s\n", recordDcCycles ? "true" : "false");
    printf("=====================================\n");
    fflush(stdout);
    
    // Also use EV for debugging
    EV << "=== SLAC APPLICATION INITIALIZE ===" << endl;
    EV << "Node " << nodeId << " (" << nodeType << "): Initializing" << endl;
    EV << "dcLoopEnabled = " << (dcLoopEnabled ? "true" : "false") << endl;
    EV << "recordSlacMessages = " << (recordSlacMessages ? "true" : "false") << endl;
    EV << "recordDcCycles = " << (recordDcCycles ? "true" : "false") << endl;
    EV << "=====================================" << endl;
    
    // Start SLAC for all nodes (testing)
    startSlac();
}
...
void SlacApplication::handleMessage(cMessage* msg)
{
    if (msg->isSelfMessage()) {
        ...
        else if (strcmp(msg->getName(), "forceStartDc") == 0) {
            startDcLoop();
        }
        delete msg;
    }
...
void SlacApplication::startSlac()
{
    ...
    // For testing: immediately complete SLAC
    printf("[%.3f] Node %d: Completing SLAC immediately for testing\n", simTime().dbl(), nodeId);
    fflush(stdout);
    onSlacDone(true);
}
...
void SlacApplication::sendSlacMessage(SlacMessageType type, int bits)
{
    ...
    // Log MAC transmission if enabled
    if (recordSlacMessages) {
        logMacTx(priority, bits, startTime, startTime + getFrameDuration(msg), true, 1, 0, 0);
    }
...
void SlacApplication::handleSlacResponse(SlacMessageType type)
{
    ...
    if (awaitingResponse && awaitingMessageType == type) {
        ...
        if (allReceived) {
            printf("[%.3f] Node %d: SLAC completed successfully, starting DC loop\n", simTime().dbl(), nodeId);
            onSlacDone(true);
        }
    }
}
...
void SlacApplication::startDcLoop()
{
    ...
    dcStarted = true;
    printf("[%.3f] Node %d: Starting DC loop\n", simTime().dbl(), nodeId);
    fflush(stdout);
...
void SlacApplication::sendDcRequest()
{
    ...
    printf("[%.3f] Node %d (%s): Sending DC request #%d\n", now.dbl(), nodeId, nodeType.c_str(), dcReqSeq);
    fflush(stdout);
    ...
    simtime_t reqTime = now;
    
    // Send DC request
    sendSlacMessage(DC_CUR_DEM_REQ, 300 * 8);
    
    // Schedule response timeout
    scheduleAt(now + dcDeadline, new cMessage("dcTimeout"));
    
    // Schedule next DC request
    scheduleAt(simTime() + dcPeriod, new cMessage("dcTick"));
    
    // Log DC cycle if enabled
    if (recordDcCycles) {
        logDcCycle(dcReqSeq, reqTime, 0, 0, false, gapViolation, 0, 3);
    }
...
void SlacApplication::logMacTx(int kind, int bits, simtime_t startTime, simtime_t endTime, bool success, int attempts, int bpc, int bc)
{
    EV << "logMacTx called: nodeId=" << nodeId << ", kind=" << kind << ", bits=" << bits << endl;
    FILE* logFile = fopen("results/mac_tx.log", "a");
    if (logFile) {
        fprintf(logFile, "%d,%d,%d,%d,%.6f,%.6f,%d,%d,%d,%d\n",
                0, // run_id placeholder
                nodeId, kind, bits, startTime.dbl(), endTime.dbl(),
                success ? 1 : 0, attempts, bpc, bc);
        fclose(logFile);
        EV << "Logged to mac_tx.log" << endl;
    } else {
        EV << "Failed to open mac_tx.log" << endl;
    }
}

void SlacApplication::logDcCycle(int seq, simtime_t reqTime, simtime_t rspTime, simtime_t rtt, bool missFlag, bool gapViolation, int retries, int segFrames)
{
    FILE* logFile = fopen("results/dc_cycle.log", "a");
    if (logFile) {
        fprintf(logFile, "%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d\n",
                0, // run_id placeholder
                nodeId, seq, 100000, // deadline_ms = 100ms
                reqTime.dbl(), rspTime.dbl(), rtt.dbl(),
                missFlag ? 1 : 0, gapViolation ? 1 : 0, retries, segFrames);
        fclose(logFile);
    }
}

void SlacApplication::logSlacAttempt(int tryId, simtime_t startTime, simtime_t endTime, bool success, simtime_t connTime, int msgTimeouts, bool procTimeout, int retries)
{
    FILE* logFile = fopen("results/slac_attempt.log", "a");
    if (logFile) {
        fprintf(logFile, "%d,%d,%d,%.6f,%.6f,%d,%.6f,%d,%d,%d\n",
                0, // run_id placeholder
                nodeId, tryId, startTime.dbl(), endTime.dbl(),
                success ? 1 : 0, connTime.dbl(), msgTimeouts, procTimeout ? 1 : 0, retries);
        fclose(logFile);
    }
}

simtime_t SlacApplication::getFrameDuration(cMessage* frame)
{
